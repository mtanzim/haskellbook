...
test = hspec $ do
  ...
  describe "handleGuessPure" $ do
    it "guessed correctly" $ do
      handleGuessPure puzzle1 char1 `shouldBe` exptected1
    it "guessed incorrectly" $ do
      handleGuessPure puzzle2 char2 `shouldBe` expected2
    it "already guessed" $ do
      handleGuessPure puzzle3 char3 `shouldBe` expected3
  where
    puzzle1 = Puzzle "cat" [Nothing, Nothing, Nothing] [] 20
    char1 = 'a'
    exptected1 = ("Word found, filling it in!", Puzzle "cat" [Nothing, Just 'a', Nothing] ['a'] 20)

    puzzle2 = puzzle1
    char2 = 'b'
    expected2 = ("Try again", Puzzle "cat" [Nothing, Nothing, Nothing] ['b'] 19)

    puzzle3 = Puzzle "cat" [Nothing, Nothing, Nothing] ['a'] 15
    char3 = 'a'
    expected3 = ("You already guessed that, pick something else", Puzzle "cat" [Nothing, Nothing, Nothing] ['a'] 15)
